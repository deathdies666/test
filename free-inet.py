...
def log_replace(value, status='', color='[Y1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/proxy.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check proxy.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('Connected! ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        '[G1]===========TEST NOL PULSA NOL KUOTA===========',
        '[R1]=============AUTHOR BY DEATHDIES=============='
    ])))
